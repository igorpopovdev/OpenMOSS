...
function formatAction(action: string) {
    return ({
        plan: 'Планирование', coding: 'Выполнение', delivery: '',
        review: 'Рецензия', patrol: 'Патрулирование', reflection: 'Рефлексия', blocked: 'Блокировка',
        ...
        <header class="shrink-0 border-b border-border/40 bg-background px-4 py-3 space-y-2.5">
            <div class="flex items-center gap-3">
                <div class="relative flex-1 max-w-sm">
                    ...
                    <Input v-model="keyword" class="h-9 bg-muted/30 pl-10 text-sm" placeholder="Поиск по содержимому…" />
                </div>
        ...
        <div class="flex-1 min-h-0 overflow-y-auto">
            ...
            <template v-else>
                    ...
                    <div v-for="(log, idx) in pageData.items" :key="log.id"
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <!-- Первая строка: Agent + Action + Время -->
                                <div class="flex items-center gap-1.5 flex-wrap">
                                    <span class="text-sm font-semibold">{{ log.agent_name }}</span>
                                    <Badge variant="outline" :class="getRoleBadgeClass(log.agent_role)" class="text-[10px] px-1.5">
                                        {{ formatRole(log.agent_role) }}
                                    </Badge>
                                    <Badge variant="outline" :class="getActionBadgeClass(log.action)" class="text-[10px] px-1.5">
                                        {{ formatAction(log.action) }}
                                    </Badge>
                                    <span class="text-[11px] text-muted-foreground/50 ml-auto shrink-0 tabular-nums">
                                        {{ formatRelativeTime(log.created_at) }}
                                        <span class="hidden sm:inline ml-1 text-muted-foreground/40">{{ formatDate(log.created_at) }}</span>
                                    </span>
                                </div>
                                ...
                                <div class="mt-1">
                                    ...
                                    <button v-if="isSummaryLong(log.summary)"
                                        ...
                                        <template v-if="expandedIds.has(log.id)">
                                            <ChevronUp class="h-3 w-3" /> Свернуть
                                        </template>
                                        <template v-else>
                                            <ChevronDown class="h-3 w-3" /> Развернуть
                                        </template>
                                    </button>
                                </div>

                                <!-- Связанная информация -->
                                <div v-if="log.sub_task_id" class="mt-1 text-[11px] text-muted-foreground/50 tabular-nums">
                                    Подзадача {{ log.sub_task_id.slice(0, 8) }}…
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div v-else class="flex flex-col items-center justify-center py-16 text-muted-foreground/40">
                    ...
                    <p class="text-sm">Нет записей активности</p>
                </div>
